...
How It Works
Read a packet from the input PCAP.

Parse its Ethernet and IPv4 headers, followed by TCP or UDP headers where applicable.

Identify its directional flow using a five-tuple:

text

source IP, destination IP, source port, destination port, protocol
Attempt hostname or domain extraction based on the destination port:

Destination port	Extraction attempted
443	TLS ClientHello SNI
80	HTTP Host: header
53	DNS query domain
Classify the flow using extracted hostnames or port-based fallbacks.

Evaluate blocking rules.

Write forwarded packets to the output PCAP.
...
Use classic microsecond-resolution PCAP captures containing Ethernet II/IPv4 traffic. See Limitations and Known Issues for unsupported formats and processing constraints.
...
This project is best treated as an educational offline DPI implementation, not a production firewall or a lossless capture-processing tool.
